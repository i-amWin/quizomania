...
function addResultCheckingFunctionality() {
  const quizAnsContainer = quizzesContainer.querySelectorAll(".quiz__answers");

  function addListener(quizAns) {
    quizAns.addEventListener("click", showResult);
  }

  Array.from(quizAnsContainer).map(addListener);

  function showResult(e) {
    const clickedAnswer =
      e.target.querySelector(".quiz__answer__btn") || e.target;
    const correctAnswer = this.nextElementSibling.querySelector("span");
    ...
    this.removeEventListener("click", showResult);
  }
}
...
async function addQuizzes() {
  ...
  addResultCheckingFunctionality();
}
